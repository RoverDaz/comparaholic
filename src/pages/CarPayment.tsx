...
const defaultFormFields = [
  {
    ...
    field_options: ['Toyota', 'Honda', 'Ford', 'Chevrolet', 'BMW', 'Mercedes', 'Hyundai', 'Kia', 'Volkswagen', 'Audi'],
    is_required: true,
    display_order: 0
  },
  {
...
export function CarPayment() {
  ...
  useEffect(() => {
    ...
    if (currentStep === 1 && formState.make) {
      console.log('Models for selected make:', carModels[formState.make]);
    }
  }, [currentStep, formState, navigate]);
  ...
  const handleSelect = async (answer: string) => {
    ...
    flushSync(() => setFullFormState(newFormState));
    await saveUserResponses('new-car-payment');
