/*
  # Fix Insurance Quotes Access and Sample Data

  1. Changes
    - Enable RLS on insurance_quotes table
    - Add policy to allow authenticated users to read quotes
    - Repopulate sample data with proper permissions
*/

-- Enable RLS on insurance_quotes table
ALTER TABLE insurance_quotes ENABLE ROW LEVEL SECURITY;

-- Create policy to allow authenticated users to read quotes
CREATE POLICY "Anyone can read insurance quotes" 
ON insurance_quotes
FOR SELECT 
TO authenticated
USING (true);
...
-- Create a more permissive policy for reading quotes
